...
def plot_pos(t, truth_pos, imu_pos, imu_pos_error, model_pos, mod_pos_error, kf_pos, kf_pos_error, navkf_pos, navkf_pos_error):

    #### position propagation ####
    fig, ax1 = plt.subplots()
    ax1.plot(truth_pos[0], truth_pos[1], c='cyan', label='TruckSim')
    ax1.plot(imu_pos[0], imu_pos[1], c='orange', label='Mechanized')
    ax1.plot(model_pos[0], model_pos[1], c='red', label='Model')
    ax1.plot(kf_pos[0], kf_pos[1], c='k', label='KF')
    ax1.plot(navkf_pos[0], navkf_pos[1], c='m', label='Nav KF')
    ax1.legend()
    ax1.set_xlabel('X [m]')
    ax1.set_ylabel('Y [m]')
    ax1.tick_params(axis='x',labelsize=13)
    ...
    ax2.plot(t, imu_pos_error, c='orange', label='Mechanized')
    ax2.plot(t, mod_pos_error, c='red', label='Model')
    ax2.plot(t, kf_pos_error, c='k', label='KF')
    ax2.plot(t, navkf_pos_error, c='m', label='Nav KF')
    ax2.legend()
