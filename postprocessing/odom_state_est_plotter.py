...
def odom_state_est_plotter(x, x_truth, t, interactive:bool=False):
    """
    Generates plots for the 12-state odom nav estimates vs truth and their errors.
    ...
    # biases
    fig, axs = plt.subplots(2,2)
    fig.suptitle('Bias Estimates')
    axs[0,0].plot(t, np.rad2deg(bias_yr))
    axs[0,0].set_ylabel('Yaw Rate Bias (deg/s)')
    axs[0,0].set_xlabel('Time [s]')
    axs[0,1].plot(t, bias_dx)
    axs[0,1].set_ylabel('dx Bias (m)')
    axs[0,1].set_xlabel('Time [s]')
    axs[1,0].plot(t, bias_dy)
    axs[1,0].set_ylabel('dy Bias (m)')
    axs[1,0].set_xlabel('Time [s]')
    axs[1,1].plot(t, np.rad2deg(bias_dyaw))
    axs[1,1].set_ylabel('dyaw Bias (rad)')
    axs[1,1].set_xlabel('Time [s]')
    plt.tight_layout()
    plt.show()
