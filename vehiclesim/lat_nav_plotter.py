...
def plot_estimator_specs(t, P, labels, innov=None, t_factor=1):
    # determine number of states from P
    n = P[0].shape[0]
    covars = []
    for P_ in P:
        covars.append(np.diag(P_))
    covars = np.array(covars).transpose()
...
def plot_tan_pos(t, truth_pos, model_pos, kf_pos, mod_pos_error, kf_pos_error, t_factor=1):

    #### position propagation ####
    fig, ax1 = plt.subplots()
    ax1.plot(truth_pos[0], truth_pos[1], c='cyan', label='Truth')
    ...
    ax2.plot(t/t_factor, mod_pos_error, c='red', label='Model')
    ax2.plot(t/t_factor, kf_pos_error, c='k', label='KF')
    # ax2.legend()
    ax2.set_xlabel('Time [min]')
