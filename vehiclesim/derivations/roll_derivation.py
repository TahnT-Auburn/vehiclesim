...
#----- Jacobian with suspension params KF Formulation -----#
F2 = Matrix([[diff(dynamics.rhs,phi_dot), diff(dynamics.rhs,phi), diff(dynamics.rhs,hr), diff(dynamics.rhs,k), diff(dynamics.rhs,c)],
            [diff(phi_dot,phi_dot), diff(phi_dot,phi), diff(phi_dot,hr), diff(phi_dot,k), diff(phi_dot,c)],
            [diff(hr_dot,phi_dot), diff(hr_dot,phi), diff(hr_dot,hr), diff(hr_dot,k), diff(hr_dot,c)],
            [diff(k_dot,phi_dot), diff(k_dot,phi), diff(k_dot,hr), diff(k_dot,k), diff(k_dot,c)],
            [diff(c_dot,phi_dot), diff(c_dot,phi), diff(c_dot,hr), diff(c_dot,k), diff(c_dot,c)]])

F2 = F2.subs(phi_dot,dphi)