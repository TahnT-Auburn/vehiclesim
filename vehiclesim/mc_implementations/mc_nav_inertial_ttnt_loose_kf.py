...
#%%
# monte carlo loop
for m in tqdm(range(0,L_MC), position=0):
    # Generate new vehicle configs for every MC
    ...
    #     L=L
    # )
    steer_meas = steer_truth + np.deg2rad(0.5)*np.random.randn(L)
    vx_meas = vx_truth + 0.01*np.random.randn(L)
    ...
    j = 0
    inertial_inputs = []
    for k in tqdm(range(0,L-1), position=1, leave=False):
        # ---- ZUPT ----
